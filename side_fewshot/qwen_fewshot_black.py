...
OUTPUT_FILE = "/projects/0/prjs1482/UvA/AA_SIDE/side_fewshot/Results/qwen_speakeronly_black.json"
...
def run_speaker_trial(
    speaker_trial_path, target_idx, model, processor, black_img, results_list
):
    ...
    # Prepare black+target image list
    speaker_images = []
    for i in range(NUM_IMAGES):
        if i == target_idx:
            speaker_images.append(Image.open(speaker_img_paths[i]).convert("RGB"))
        else:
            speaker_images.append(black_img)
        ...
        for folder in speaker_folders:
            ...
            if os.path.isdir(speaker_trial_path):
                run_speaker_trial(
                    ...
                    model, processor, black_img, all_results
                )
